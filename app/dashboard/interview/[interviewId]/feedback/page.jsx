...
function Feedback({params}) {
  ...
  return (
    <div className='p-10'>
      ...
      {feedbackList?.length === 0 ? <h2 className='font-bold text-xl text-gray-500 '>No Interview Taken</h2>
      :
      <>
        ...
        <CollapsibleTrigger className='p-2 bg-secondary rounded-lg my-2  text-left flex justify-between gap-7 w-full'>{item.question}<ChevronsUpDown className='h-5 w-5'/></CollapsibleTrigger>
         <CollapsibleContent>
            <div className='flex flex-col gap-2'>
              ...
              <h2 className='p-2 border rounded-lg bg-blue-50 text-sm text-blue-900'><strong>AI Feedback:</strong>{item.feedback}</h2>
            </div>
         </CollapsibleContent>
         </Collapsible>
      ))}
