...
function QuestionSections({MockInterviewQuestions,activeQuestionIndex}) {
  ...
  const textToSpeach = (text) => {
    if('speechSynthesis' in window){
      ...
      alert('Sorry!! Your Browser doesnot supprt text to speech');
    }
  }
      ...
      <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5'>
        {
            MockInterviewQuestions.map((question,index) => (
                <h2  className={`p-2 bg-secondary rounded-full text-xs md:text-sm text-center cursor-pointer ${index === activeQuestionIndex ? 'bg-violet-700 text-white' : ''}`}>Question #{index + 1}</h2>
            ))
        }
